          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="text-left text-white/70 border-b border-white/10">
                <th className="py-4 pr-4 font-medium">Comparison</th>
                <th className="py-4 pr-4 font-medium">Air Quality</th>
                <th className="py-4 pr-4 font-medium">Weather Apps</th>
                <th className="py-4 pr-4 font-medium">Asthma Care</th>
                <th className="py-4 font-medium text-white">Satori</th>
              </tr>
            </thead>
...
function Row({ title, cols }: { title: string; cols: string[] }) {
  return (
    <tr className="border-b border-white/5 align-top">
      <td className="py-6 pr-4 text-white font-medium w-[18%]">{title}</td>
      {cols.map((c, i) => (
        <td key={i} className="py-6 pr-4 text-white/75 w-[20%] leading-relaxed">
          {c}
        </td>
      ))}
    </tr>
